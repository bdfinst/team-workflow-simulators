...
**CRITICAL: After making ANY code change, you MUST verify quality by running all
three checks.**

This is NON-NEGOTIABLE. Every change must pass all quality gates before being
considered complete.
...
If ANY check fails, the change is NOT complete. Fix the issues and re-run all
checks.
...
2. Some errors can be auto-fixed with `npm lint --fix`
