  ...
  <label class="flex min-w-[120px] flex-1 flex-col gap-0.5">
    <span class="flex items-center justify-between text-xs text-surface-300">
      <span class="flex items-center">
        Batch Multiplier
        <ControlTooltip
          text="How many times larger a monolithic item is vs a thin slice"
        />
      </span>
      <span class="font-mono text-accent-cyan">{config.batchMultiplier}x</span>
    </span>
    <input
      type="range"
      min="1"
      max="10"
      value={config.batchMultiplier}
      aria-valuetext="{config.batchMultiplier} times larger"
      oninput={(e) => handleChange('batchMultiplier', e)}
      disabled={isRunning}
      class="h-1 w-full cursor-pointer accent-accent-cyan"
      data-testid="batch-multiplier-input"
    />
  </label>
