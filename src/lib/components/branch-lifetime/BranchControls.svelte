  ...
  <label class="flex min-w-[120px] flex-1 flex-col gap-0.5">
    <span class="flex items-center justify-between text-xs text-surface-300">
      <span class="flex items-center">
        Branch Lifetime
        <ControlTooltip
          text="How many days a feature branch lives before merging"
        />
      </span>
      <span class="font-mono text-accent-cyan">{config.branchLifetime}d</span>
    </span>
    <input
      type="range"
      min="1"
      max="14"
      value={config.branchLifetime}
      aria-valuetext="{config.branchLifetime} days"
      oninput={(e) => handleChange('branchLifetime', e)}
      disabled={isRunning}
      class="h-1 w-full cursor-pointer accent-accent-cyan"
      data-testid="branch-lifetime-input"
    />
  </label>
  ...
  <label class="flex min-w-[120px] flex-1 flex-col gap-0.5">
    <span class="flex items-center justify-between text-xs text-surface-300">
      <span class="flex items-center">
        Code Overlap
        <ControlTooltip
          text="How much of the codebase is modified by concurrent branches"
        />
      </span>
      <span class="font-mono text-accent-cyan"
        >{Math.round(config.codeOverlap * 100)}%</span
      >
    </span>
    <input
      type="range"
      min="0"
      max="100"
      value={config.codeOverlap * 100}
      aria-valuetext="{Math.round(config.codeOverlap * 100)} percent overlap"
      oninput={(e) => onupdate('codeOverlap', Number(e.target.value) / 100)}
      disabled={isRunning}
      class="h-1 w-full cursor-pointer accent-accent-cyan"
      data-testid="code-overlap-input"
    />
  </label>
