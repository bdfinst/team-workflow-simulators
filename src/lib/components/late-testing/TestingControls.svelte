...
>
  <label class="flex min-w-[110px] flex-1 flex-col gap-0.5">
    <span class="flex items-center justify-between text-xs text-surface-300">
      <span class="flex items-center">
        Defect Rate
        <ControlTooltip
          text="Maximum rework cycles due to defects found during testing"
        />
      </span>
      <span class="font-mono text-accent-cyan">{config.defectRate}</span>
    </span>
    <input
      type="range"
      min="0"
      max="4"
      value={config.defectRate}
      aria-valuetext="{config.defectRate} rework cycles"
      oninput={(e) => handleChange('defectRate', e)}
      disabled={isRunning}
      class="h-1 w-full cursor-pointer accent-accent-cyan"
      data-testid="defect-rate-input"
    />
  </label>
  ...
  <label class="flex min-w-[110px] flex-1 flex-col gap-0.5">
    <span class="flex items-center justify-between text-xs text-surface-300">
      <span class="flex items-center">
        Feedback Delay
        <ControlTooltip
          text="Extra delay when testing happens only at the end"
        />
      </span>
      <span class="font-mono text-accent-cyan">{config.testFeedbackDelay}t</span
      >
    </span>
    <input
      type="range"
      min="0"
      max="8"
      value={config.testFeedbackDelay}
      aria-valuetext="{config.testFeedbackDelay} ticks delay"
      oninput={(e) => handleChange('testFeedbackDelay', e)}
      disabled={isRunning}
      class="h-1 w-full cursor-pointer accent-accent-cyan"
      data-testid="feedback-delay-input"
    />
  </label>
