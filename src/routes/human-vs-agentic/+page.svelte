<script>
  import { humanVsAgenticStore } from '$lib/stores/humanVsAgenticStore.svelte.js'
  import PipelineView from '$lib/components/simulator/PipelineView.svelte'
  import AgenticControls from '$lib/components/human-vs-agentic/AgenticControls.svelte'
  import ConfigPanel from '$lib/components/simulator/ConfigPanel.svelte'
  import AgenticMetrics from '$lib/components/human-vs-agentic/AgenticMetrics.svelte'
  import AgenticCallouts from '$lib/components/human-vs-agentic/AgenticCallouts.svelte'

  const sim = humanVsAgenticStore
...
<main
  ...
  data-testid="human-vs-agentic-page"
>
  <!-- Top bar: title + nav -->
  ...
  <div class="mt-3 grid min-h-0 flex-1 grid-cols-1 gap-3 xl:grid-cols-4">
    ...
    >
      <PipelineView
        pipeline={sim.humanPipeline}
        label="Human Development"
        variant="danger"
        isRunning={sim.isRunning}
      />
      <PipelineView
        pipeline={sim.agentManualPipeline}
        label="Agent + Manual Review"
        variant="neutral"
        isRunning={sim.isRunning}
      />
      <PipelineView
        pipeline={sim.agentAutoPipeline}
    ...
    <aside class="flex min-h-0 flex-col gap-4 overflow-y-auto xl:col-span-1">
      <AgenticMetrics
        humanMetrics={sim.humanMetrics}
        agentManualMetrics={sim.agentManualMetrics}
        agentAutoMetrics={sim.agentAutoMetrics}
